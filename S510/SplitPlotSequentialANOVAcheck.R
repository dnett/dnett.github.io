...
o=lm(y~block+g+block:g+f+geno:f)
anova(o)


library(MASS)
x1=matrix(1,nrow=48,ncol=1)
x2=cbind(x1,model.matrix(~0+block))
x3=cbind(x2,model.matrix(~0+g))
x4=cbind(x3,model.matrix(~0+block:g))
x5=cbind(x4,model.matrix(~0+f))
x6=cbind(x5,model.matrix(~0+g:f))
...
t(y)%*%(p5-p4)%*%y
t(y)%*%(p6-p5)%*%y
t(y)%*%(diag(rep(1,48))-p6)%*%y

xg=cbind(x1,model.matrix(~0+g))
pg=proj(xg)
t(y)%*%(pg-p1)%*%y
#Same as
t(y)%*%(p3-p2)%*%y
